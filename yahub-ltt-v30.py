...
def load_env(path=".env"):
    ...
    if not os.path.exists(path):
        logger.error("Khong tim thay file .env!")
        exit(1)
...
BOT_OWNERS = [uid.strip() for uid in env.get("BOT_OWNERS", "").split(",") if uid.strip()]
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            for idx, match in enumerate(matches):
                ...
                for team in ranks:
                    if team.get("rank") == 1:
                        booyah_team = team.get("teamName") or team.get("accountNames", ["Unknown"])[0]
                        break
