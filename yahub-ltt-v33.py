...
def render_image(bg_name, leaderboard, start_str, name_str, logo_bytes, logo_map={}, match_details=None):
    try:
        importlib.invalidate_caches()
        renderer = importlib.import_module(f"renderers.{bg_name}")
        import inspect
        sig = inspect.signature(renderer.create_image)
        if "match_details" in sig.parameters:
            return renderer.create_image(leaderboard, start_str, name_str, logo_bytes, logo_map, match_details)
...
async def list_bg(interaction: discord.Interaction):
    ...
    # FIX: try/except cho defer phòng interaction hết hạn
    try:
    ...
    # FIX: Hiển thị danh sách tên trước
...
async def bxh(
    ...
    # FIX: Bọc defer trong try/except để tránh lỗi 10062 (Unknown interaction)
    ...
    try:
        ...
        pass  # Đã respond rồi, tiếp tục bình thường
    except Exception as e:
    ...
    try:
        ...
        # FIX: Dùng chung 1 aiohttp session cho toàn bộ request (thay vì tạo 2 session riêng)
                ...
                for team in ranks:
                    ...
                    if team_name and team_name.strip() != "":
                        ...
                        for keyname in team_map:
                            ...
                            # Ngưỡng >= 2 khiến đội chỉ còn 3 người (thiếu 1 người so với
                            # đội gốc 4 người) dễ bị coi là đội mới nếu phần trùng < 2,
                            # dẫn tới điểm bị tách ra thay vì cộng dồn.
                            if len(common) >= 1:
                                found_key = keyname
                                break
                        ...
                        if found_key:
                            ...
                            merged_ids = list(team_map[found_key].get("accountIds", []))
                            for _id in current_ids:
                                if _id not in merged_ids:
                                    merged_ids.append(_id)
                            team_map[found_key]["accountIds"] = merged_ids
        ...
        image = render_image(background, leaderboard, start_time, custom_name, logo_bytes, logo_map if add_logo else {}, match_details=match_details)
